package org.learning.lld.practice_problems.parking_lot.parkingStrategy;

import org.learning.lld.practice_problems.parking_lot.ParkingLot;
import org.learning.lld.practice_problems.parking_lot.ParkingSpot;
import org.learning.lld.practice_problems.parking_lot.VechileType.VechileType;

public class NearestToExit implements ParkingSpotAllocationStrategy {
    @Override
    public ParkingSpot getAvailableParkingSpot(ParkingLot parkingLot, VechileType vechileType) {
        System.out.println("finding available parking spot using NearestToExit algorithm.");
        var spot = new ParkingSpot(12, vechileType);
        System.out.printf("spot allocated: %s\n", spot);
        return spot;
    }
}
